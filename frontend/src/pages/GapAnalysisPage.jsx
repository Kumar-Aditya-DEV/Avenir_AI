...
export default function GapAnalysisPage() {
  ...
  const matchedSkills = Array.isArray(analysisData?.matchedSkills) ? analysisData.matchedSkills : [];
  ...
  const dynamicBarSkills = [
    ...matchedSkills.slice(0, 5).map(s => ({ name: s, percentage: 100, level: 'Matched', color: '#10B981' })),
    ...missingSkills.slice(0, 5).map(s => ({ name: s, percentage: 15, level: 'Missing', color: '#EF4444' }))
  ];

  const dynamicHeatMapData = [
    ...matchedSkills.map(s => ({ category: 'Matched', skill: s, match: 100, status: 'green' })),
    ...missingSkills.map(s => ({ category: 'Missing', skill: s, match: 0, status: 'red' }))
  ];
            ...
            <div className="inline-flex bg-[#F3F4F6] rounded-xl p-1 relative z-10 shrink-0">
              {['bar', 'heatmap'].map((tab) => (
                <button
                  ...
                  className={`relative px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors z-10
          ...
          <div className="min-h-[260px] flex items-center justify-center">
            <AnimatePresence mode="wait">
              {visTab === 'bar' && (
                ...
                >
                  {dynamicBarSkills.map((item, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                      <div className="w-28 text-xs font-bold text-[#111827]">{item.name}</div>
                      <div className="flex-1 flex justify-end">
                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase
                          ${item.level === 'Matched' ? 'bg-[#ECFDF5] text-[#10B981]' : 'bg-[#FEF2F2] text-[#EF4444]'}
                        `}>
                          {item.level}
                        </span>
                      </div>
              ...
              {visTab === 'heatmap' && (
                ...
                >
                  {dynamicHeatMapData.map((cell, idx) => (
                    <motion.div
                      key={idx}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-3 pt-2 border-t border-black/5">
                        <span className="text-[10px] font-bold uppercase text-[#6B7280]">Status</span>
                        <span className={`text-xs font-extrabold uppercase
                          ${cell.status === 'green' ? 'text-emerald-700' :
                            cell.status === 'amber' ? 'text-amber-700' : 'text-red-700'}
                        `}>
                          {cell.status === 'green' ? 'Matched' : 'Missing'}
                        </span>
                      </div>
                      ...
                      {hoveredCell === cell && (
                        <div className="absolute left-1/2 -top-12 -translate-x-1/2 z-40 bg-white border border-[#E5E7EB] rounded-lg px-2.5 py-1.5 shadow-lg text-[10px] font-bold text-[#111827] whitespace-nowrap pointer-events-none">
                          <p>{cell.skill}</p>
                          <p className={`mt-0.5 ${cell.status === 'green' ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
                            {cell.status === 'green' ? '✓ Matched Requirement' : '! Missing Requirement'}
                          </p>
                        </div>
                      )}
                    </motion.div>
                        ...
                        >
                          <div className="bg-white rounded-xl border border-[#E5E7EB] p-4 shadow-sm text-xs font-medium text-[#111827]">
                            <div className="flex items-start gap-2.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-[#EF4444] mt-1.5 shrink-0" />
                              <div>
                                <p className="font-bold">Missing Core Skills</p>
                                <p className="text-[11px] text-[#6B7280] mt-0.5">Your resume lacks some core technical requirements mentioned in the JD.</p>
                              </div>
                            </div>
                          </div>
                        </motion.div>
                    ...
                    {missingSkills.slice(0, 3).map((skill, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: idx % 2 === 0 ? '#EFF6FF' : '#F5F3FF' }}>
                            <GraduationCap size={15} style={{ color: idx % 2 === 0 ? '#2563EB' : '#7C3AED' }} />
                          </div>
                          <div>
                            <h4 className="text-xs font-bold text-[#111827]">Mastering {skill}</h4>
                            <p className="text-[10px] text-[#6B7280] mt-0.5">Crash course on core concepts</p>
                          </div>
                        </div>
                ...
                <ul className="space-y-3.5 text-xs text-[#111827] font-medium">
                  <li className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[#6B7280] ml-1">You possess strong skills in {matchedSkills.slice(0, 2).join(', ') || 'key areas'}.</span>
                    </div>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="text-[#10B981] mt-0.5">✓</span>
                    <div>
                      <span className="font-bold">ATS Compatibility:</span>
                      <span className="text-[#6B7280] ml-1">Your resume format parsed successfully with {atsScore}% match.</span>
                    </div>
                  </li>
                </ul>
              </div>
                ...
                <ul className="space-y-3.5 text-xs text-[#111827] font-medium">
                  <li className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[#6B7280] ml-1">Lack of credentials in {missingSkills.slice(0, 2).join(', ') || 'some JD requirements'}.</span>
                    </div>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="text-[#F59E0B] mt-0.5">!</span>
                    <div>
                      <span className="font-bold">Role Alignment:</span>
                      <span className="text-[#6B7280] ml-1">Partial matching in key developer frameworks or tools.</span>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <motion.div variants={itemVar} className="bg-white rounded-2xl shadow-[0_10px_30px_rgba(15,23,42,0.08)] border border-[#E5E7EB] p-6 lg:p-8">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {missingSkills.slice(0, 10).map((kw, i) => (
                  <motion.span
                    key={kw}
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[4fr_6fr] gap-6">
              ...
              <motion.div variants={itemVar} className="bg-white rounded-2xl shadow-[0_10px_30px_rgba(15,23,42,0.08)] border border-[#E5E7EB] p-6">
                ...
                <div className="relative border-l border-[#E5E7EB] ml-3.5 pl-6 space-y-6">
                  {[
                    { title: 'Current State', desc: `Match score at ${atsScore}% with critical gaps.`, status: 'current', time: 'Today' },
                    { title: `Learn ${missingSkills[0] || 'Core Concepts'}`, desc: `Master the basics of ${missingSkills[0] || 'the role'}.`, status: 'pending', time: 'Step 1' },
                    { title: `Practice ${missingSkills[1] || 'Advanced Patterns'}`, desc: 'Build scalable integrations.', status: 'pending', time: 'Step 2' },
